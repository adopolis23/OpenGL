...
Renderer::Renderer(const Camera *cam) : camera(cam) {
  ...
  densityShader = new Shader("src/shaders/vertex/density_vertex.glsl",
                             "src/shaders/fragment/density_fragment.glsl");
}
...
void Renderer::InitDensityResources(const DensityField &densityField) {
  ...
  glTexImage2D(GL_TEXTURE_2D, 0, GL_R32F, densityField.width,
               densityField.height, 0, GL_RED, GL_FLOAT,
...
void Renderer::UploadDensity(const DensityField &densityField) {
  ...
  glBindTexture(GL_TEXTURE_2D, densityTexture);
  glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, densityField.width,
                  densityField.height, GL_RED, GL_FLOAT,
...
void Renderer::UploadParticlePositions(const Scene &scene, float kernelRadius) {
  ...
  glBufferData(GL_SHADER_STORAGE_BUFFER,
               ...
               gpuParticlePositions.data(), GL_DYNAMIC_DRAW);
}
...
void Renderer::RenderDensityBackground() {
  ...
  densityShader->setVec2("worldMin", glm::vec2(camera->left_world_bound,
                                               camera->bottom_world_bound));
  densityShader->setVec2("worldMax", glm::vec2(camera->right_world_bound,
                                               camera->top_world_bound));
